...
#' @examples
#' setPartitions(4)
#' setPartitions.restricted(num=4, group=NULL, apart=c(2,3))
#' @export setPartitions
#' @export setPartitions.restricted
#'

#' @rdname setPartitions
setPartitions <- function(num){
  Set <- as.matrix(partitions::setparts(num))
  Set <- t(apply(Set, 2, convert.cano.graycode))
  return(Set)
}

#' @rdname setPartitions
setPartitions.restricted <- function(num, group=NULL, apart=NULL){
